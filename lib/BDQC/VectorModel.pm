package BDQC::VectorModel;

###############################################################################
# Class       : BDQC::VectorModel
#
...
###############################################################################
  ...
  foreach my $datum ( @{$vector} ) {
    ...
    elsif ( $datum =~ /^\s*NaN\s*$/i ) { $dataType = 'integer'; }
    ...
    #### Keep a hash of all the observed values
    $observedValues{$datum}++;
